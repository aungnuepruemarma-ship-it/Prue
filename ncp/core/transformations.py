...
def make_update_entity(entity_id: str, state_patch: dict[str, Any] | None = None, knowledge_patch: dict[str, Any] | None = None):
    def _exec(universe: Universe) -> Universe:
        ...
        entity.version += 1
        universe.version += 1
        universe.record_history({"op": "update_entity", "id": entity_id, "state_patch": state_patch or {}, "knowledge_patch": knowledge_patch or {}})
        return universe
    return _exec
...
def make_merge_entities(source_id: str, target_id: str, merged_name: str):
    def _exec(universe: Universe) -> Universe:
        ...
        uid = new_id("m")
        merged_state = {**s.state, **t.state}
        merged_knowledge = {**s.knowledge, **t.knowledge}
        universe.add_entity(Entity(
            id=uid,
        ...
        universe.version += 1
        universe.record_history({"op": "merge", "sources": [source_id, target_id], "result": uid})
        return universe
    return _exec
